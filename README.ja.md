...
| **Requesty** | 700以上のモデル（単一API） | [app.requesty.ai](https://app.requesty.ai/api-keys) | - |
...
スタンドアロンWebUIのオプション実行モードです。**APIキー**と**CLIアカウント**（`cli_account`）の両方で使えます。composerのモデルセレクター横にある**Action Plan**トグルで有効にします。
...
計画はツール呼び出しカードの上にライブToDoリストとして表示され、ステップごとのステータス（`pending` → `running` → `done` / `error`）が確認できます。計画はチャット履歴に保存され、リロードしても残ります。トグルはデフォルトで有効で、Action Planを無効にしている間は既存のエージェントフローに影響しません。
...
- **Action Plan（ベータ）** — WebUIのオプトイン計画→実行モード（APIキーまたはCLIアカウント）：1回の計画呼び出し、直接ツール実行、失敗時の限定的修復
- **WindowsとmacOSの両方で動作**
